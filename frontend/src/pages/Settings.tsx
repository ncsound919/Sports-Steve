    ...
    <div className="space-y-8">
      ...
      <GlassCard>
        ...
        <div className="grid grid-cols-3 gap-3">
          {([
            { id: 'conservative' as const, label: 'Conservative', desc: 'Small bets, steady growth', kelly: '1/4 Kelly', color: 'win' },
            { id: 'balanced' as const, label: 'Balanced', desc: 'Moderate risk and reward', kelly: '1/3 Kelly', color: 'yellow-500' },
            { id: 'aggressive' as const, label: 'Aggressive', desc: 'Larger bets, higher variance', kelly: '1/2 Kelly', color: 'loss' },
          ]).map((profile) => (
            <button
              key={profile.id}
      ...
      <GlassCard>
        ...
        {isBeginner && (
          <p className="text-xs text-text-secondary mb-4">
            Set maximum amounts to protect your bankroll. Sports Steve will stop when limits are reached.
          </p>
        )}
        <div className="grid grid-cols-3 gap-4">
          {([
            ...
            <div key={limit.label}>
              <label className="block text-xs text-text-muted mb-1.5">{limit.label} Limit ($)</label>
              <input
                type="number"
                value={limit.value}
      ...
      <GlassCard className="border-border-glass-hover">
        ...
        <div>
          ...
          <input
            type="url"
            value={draymondUrl}
            onChange={(e) => setDraymondUrl(e.target.value)}
        ...
        <div className="flex items-center gap-2 mt-3">
          <div className={`w-2 h-2 rounded-full ${draymondUrl ? 'bg-yellow-500' : 'bg-text-muted'}`} />
          <span className="text-[10px] text-text-muted">
            {draymondUrl ? 'Not connected — verify URL and start Draymond' : 'No orchestrator URL configured'}
          </span>
        </div>
      </GlassCard>
